...
function HookImage_textDownloadModifydownloadfile()
	...
	# Locate imagemagick.
        ...
        $createolcommand = $convert_fullpath . ' -background "#000" -fill white -gravity "' . escapeshellarg($image_text_position) . '" -font "' . escapeshellarg($image_text_font) . '" -size ' . escapeshellarg($width)  . 'x' . escapeshellarg($olheight) . ' caption:" ' . escapeshellarg($overlaytext) .  '  " ' . escapeshellarg($tmpolfile);
